...
/**
 * Fetch and log ERC20 token balance for a specific address
 * @param contract - Ethers Contract instance
 * @param address - Wallet/Contract address to check balance for
 */
const getTokenBalance = async (contract: Contract, address: string): Promise<void> => {
    try {
        //const balance: bigint = await (contract.balanceOf as (addr: string) => Promise<bigint>)(address);
        const balance: bigint = await contract.balanceOf!(address);
        const symbol: string = await contract.symbol!();
        // USDT/USDC use 6 decimals - explicitly use 6 instead of dynamic decimals for consistency
        console.log(`Balance of ${address}: ${ethers.formatUnits(balance, 6)} ${symbol}`);
    } catch (error) {
        const errorMsg = error instanceof Error ? error.message : "Unknown error";
        console.error("An error occurred in getTokenBalance:", errorMsg);
    }
};

/**
 * Read Tether (USDT) contract data from specified RPC endpoint
 * @param rpc_url - Ethereum RPC URL
 */
const readTetherContract = async (rpc_url: string): Promise<void> => {
    let start: number = Date.now();
    try {
        // Tether: USDT Stablecoin on Ethereum Mainnet
        const tetherContractAddress: string = "0xdAC17F958D2ee523a2206206994597C13D831ec7";
        ...
        // Tether: USDT Stablecoin on Ethereum Mainnet - JSON ABI (type annotated)
        const json_abi: readonly ethers.JsonFragment[] = [
        ...
        const tetherContract: Contract = new ethers.Contract(tetherContractAddress, human_readable_abi, provider);
        const tetherContract2: Contract = new ethers.Contract(tetherContractAddress, json_abi, provider);
        
        const symbol: string = await tetherContract.symbol!();
        console.log("Token symbol:", symbol);
        
        const decimals: number = await tetherContract.decimals!();
        console.log("Token decimals:", decimals);
        
        const totalSupply: bigint = await tetherContract.totalSupply!();
        ...
        const errorMsg = error instanceof Error ? error.message : "Unknown error";
        console.error("An error occurred in readTetherContract:", errorMsg);
    }
...
/**
 * Read USDC Stablecoin contract data from specified RPC endpoint
 * @param rpc_url - Ethereum RPC URL
 */
const readUsdcStableCoinContract = async (rpc_url: string): Promise<void> => {
    let start: number = Date.now();
    try {
        // USDC Stablecoin on Ethereum Mainnet
        const contractAddress: string = "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48";
        ...
        const contract: Contract = new ethers.Contract(contractAddress, human_readable_abi, provider);
        ...
        const errorMsg = error instanceof Error ? error.message : "Unknown error";
        console.error("An error occurred in readUsdcStableCoinContract:", errorMsg);
    }
...
/**
 * Main function to execute contract reads
 */
const main = async (): Promise<void> => {
    // Read tether contract: USDT Stablecoin on Ethereum Mainnet
    await readTetherContract(rpc_urls[0] as string);
    
    // Read USDC Stablecoin on Ethereum Mainnet
    await readUsdcStableCoinContract(rpc_urls[0] as string);
};

// Execute main function and handle top-level errors
main().catch((error) => {
    const errorMsg = error instanceof Error ? error.message : "Unknown top-level error";
    console.error("Main function failed:", errorMsg);
    process.exit(1);
});