...
// Listen for USDT Transfer events on Ethereum Mainnet (Polling approach)
...
const queryMainnetEventPulling = async (): Promise<void> => {
    try {
        ...
        const watchEvent = async (): Promise<void> => {
            ...
            try {
                ...
                for (const event of events) {
                    // Type guard to ensure event has args and is of type EventLog
                    if (!(event instanceof EventLog)) continue; // Guard clause for missing args    
...
const main = async (): Promise<void> => {
    ...
    await queryMainnetEventPulling(); // Uncomment to use polling approach
