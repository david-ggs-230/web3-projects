...
contract MyNFTMarketplace is ReentrancyGuardTransient {
    ...
    mapping(address => mapping(uint256 => uint256)) public tokenIdToLatestMarketItemId;
    
    MarketItem public constant EMPTY_MARKET_ITEM = MarketItem(
        0, address(0), 0, payable(address(0)),payable(address(0)),payable(address(0)), 0, false, false
    );
    ...
    event ListingFeeUpdated(uint256 oldPrice, uint256 newPrice, address updatedBy);
     ...
     */
    function updateListingFee(uint256 _newPrice) public onlyOwner nonReentrant {
    ...
    function createMarketItem(
        ...
        require(IERC165(nftContractAddress).supportsInterface(0x80ac58cd), "Contract must be ERC721");
        
        // Verify caller owns the NFT
        require(IERC721(nftContractAddress).ownerOf(tokenId) == msg.sender, "Only token owner can list");
        
        // Transfer listing fee to marketplace owner
        (bool feeTransfer, ) = owner.call{value: msg.value}("");
        require(feeTransfer, "Fee transfer failed");
        
        _marketItemIds.increment();
        uint256 marketItemId = _marketItemIds.current();
        
        address creator = MyNFT(nftContractAddress).getTokenCreatorById(tokenId);
    ...
    function cancelMarketItem(uint256 marketItemId) public payable nonReentrant {
        // 1. VALIDATION CHECKS
        require(marketItemId > 0 && marketItemId <= _marketItemIds.current(), "Market item does not exist");
        ...
        IERC721(item.nftContractAddress).safeTransferFrom(address(this), msg.sender, item.tokenId);
        
        // Refund listing fee if possible
        // (bool feeRefund, ) = payable(msg.sender).call{value: listingFee}("");
        // require(feeRefund, "Fee refund failed");
        
        // 4. EMIT EVENT
        emit MarketItemCanceled(marketItemId, item.nftContractAddress, item.tokenId, msg.sender);
     ...
     */
    function getLatestMarketItemByTokenId(address nftContractAddress, uint256 tokenId) public view returns (MarketItem memory, bool) {
        
        uint256 marketItemId = tokenIdToLatestMarketItemId[nftContractAddress][tokenId];
     ...
     */
    function createMarketSale(uint256 marketItemId) public payable nonReentrant {
        // 1. VALIDATION CHECKS
        require(marketItemId > 0 && marketItemId <= _marketItemIds.current(), 
     ...
     */
function relistMarketItem(uint256 marketItemId, uint256 price) public payable nonReentrant {
    // 1. VALIDATION CHECKS
    ...
    require(item.sold == true || item.canceled == true, "Item must be purchased or canceled before relisting");
    // Transfer listing fee to marketplace owner
    (bool feeTransfer, ) = owner.call{value: msg.value}("");
    require(feeTransfer, "Fee transfer failed");
    ...
    IERC721(item.nftContractAddress).safeTransferFrom(msg.sender, address(this), item.tokenId);
     ...
     */
    function fetchListedMarketItems() public view returns (MarketItem[] memory) {
...
function fetchOwnedMarketItems() public view returns (MarketItem[] memory) {
    ...
    for (uint256 i = 1; i <= totalItemCount; i++) {
        MarketItem storage item = marketItemIdToMarketItem[i];
        if ((item.owner == address(0)&& item.seller == msg.sender)|| (item.canceled && item.owner == msg.sender)) {
            temp[itemCount++]=item;
        }
    }
 ...
 */
function fetchOwnSellingMarketItems() public view returns (MarketItem[] memory) {
    uint256 totalItemCount = _marketItemIds.current();
 ...
 */
function fetchOwnPurchasedMarketItems() public view returns (MarketItem[] memory) {
    uint256 totalItemCount = _marketItemIds.current();
